...
export const RulesPreviewCopyDownloadActions: React.FC<RulesPreviewCopyDownloadActionsProps> = ({
  ...
  const handleDownload = () => {
    const markdown = singleFile ? rulesContent[0].markdown : rulesContent.map((content) => content.markdown).join('\n\n');
  ...
  return (
    <Fragment>
      ...
      >
        ...
        {showCopiedMessage ? `${isMultiFileEnvironment && !singleFile ? 'All copied!' : 'Copied!'}` : `Copy${isMultiFileEnvironment && !singleFile ? ' all' : ''}`}
      </button>
      <button
      ...
      >
        ...
        Download{isMultiFileEnvironment && !singleFile ? ' all' : ''}
      </button>
    </Fragment>
  );
};
